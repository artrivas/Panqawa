...
A vertex is called central in $G$ if the distance of any vertex is as small as possible. This is the radius of $G$. One of its property is: $$rad(G) \leq diam(G) \leq 2rad(G)$$
> **Proposition 1.3.3.** A graph $G$ of radius at most $k$ and maximum degree at most $d \geq 3$ has fewer than $\frac{d}{d-2}(d-1)^k$ vertices.
> 	Proof:
> 		Let $z$ be a central vertex of $G$, and let $D_i$ denote the set of vertices of $G$ at distance $i$ from $z$. It is easy to see that $|D_0| = 1$ and $|D_1| \leq d$. For $i \geq 1$ we have $|D_{i+1}| \leq d(d-1)^i$ for all $i < k$ by induction.
> 			_{i+1}| \leq d$
> 			